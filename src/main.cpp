...
const string strMessageMagic = "HamstersDB Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in HamstersDB
    bool fStrictPayToScriptHash = true; // Always active in HamstersDB
...
// if someone created a tampered bootstrap.dat 
	...
	if (!lessAggressive)
	{
		...
		// by Simone: no one ever broadcast any checkpoint as SPB is fully decentralized coin, so this part is fully commented
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "HamstersDB", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (!vRecv.empty())
        {
          ...
          int iSubVer = pfrom->strSubVer.find("HamstersDB");
          int iSubVerSpb = pfrom->strSubVer.find("HamstersDB");			// By Simone: HamstersDB re-brand: start to accept these as well
          if ((iSubVer < 1) && (iSubVerSpb < 1))
          {
