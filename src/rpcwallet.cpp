...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("rules",         nPaladinRuleHeight));
    obj.push_back(Pair("rules_synced",  !IsInitialRuleDownload()));
    return obj;
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new HamstersDB address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current HamstersDB address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <HamstersDB-address> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid HamstersDB address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
		"sendtoaddress <HamstersDB-address> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <HamstersDB-address> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <HamstersDB-address> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <HamstersDB-address> [minconf=1]\n"
            "Returns the total amount received by <HamstersDB-address> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
		"sendfrom <fromaccount> <toHamstersDB-address> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid HamstersDB address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a HamstersDB address or hex-encoded public key\n"
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("HamstersDB-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("HamstersDB-lock-wa");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; HamstersDB server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <HamstersDB-address>\n"
            "Return information about <HamstersDB-address>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <HamstersDB-pubkey>\n"
            "Return information about <HamstersDB-pubkey>.");
...
// HamstersDB: resend unconfirmed wallet transactions
...
Value zapwallettxes(const Array& params, bool fHelp)
{
  ...
  if (nLoadWalletRet != DB_LOAD_OK)
  {
    ...
    {
      printf("Error loading wallet.dat: Wallet requires newer version of HamstersDB\n");
      return("Error loading wallet.dat: Wallet requires newer version of HamstersDB\n");
    }
    else if (nLoadWalletRet == DB_NEED_REWRITE)
    {
      printf("Wallet needed to be rewritten: restart HamstersDB to complete\n");
      return("Wallet needed to be rewritten: restart HamstersDB to complete\n");
    }
    else
    {
