...
void SkinsPage::getlist()
{
  ...
  statusLabel->setText("<b>" + tr("Downloading themes from http://HamstersDB.co...") + "</b>");
  latestNetError = "";
  ...
  request.setUrl(QUrl("http://HamstersDB.co/themes/list.v2.txt"));
...
// calculate the md5 of remote and local files
	if (netHandleError(reply, "http://HamstersDB.co/themes/list.v2.txt"))
	{
...
void SkinsPage::getListFinished(QNetworkReply* reply)
{
	...
	// saves also the descriptor, will be used to check for an new version
		download((QString)"http://HamstersDB.co/themes/list.v2.txt");
		for (int i = 0; i < list.count(); i++)
		{
			...
			if (line.length())
			{
				...
				{  
					download("http://HamstersDB.co/themes/" + line);
				}
			}
		}
	}
